...
CLASS zcl_itab_combination IMPLEMENTATION.
  ...
  METHOD perform_combination.
    DATA(lv_alphas_count) = lines( alphas ).
    DATA(lv_nums_count) = lines( nums ).

    IF lv_alphas_count <> lv_nums_count.
      WRITE 'Number of rows in alphas != nums !!!'.

    ELSE.
      DO lv_alphas_count TIMES.
        APPEND VALUE #(
          colx = alphas[ sy-index ]-cola && nums[ sy-index ]-col1
          coly = alphas[ sy-index ]-colb && nums[ sy-index ]-col2
          colz = alphas[ sy-index ]-colc && nums[ sy-index ]-col3
        ) TO combined_data.
      ENDDO.
    ENDIF.
  ENDMETHOD.
