...
CLASS zcl_itab_aggregation IMPLEMENTATION.
  METHOD perform_aggregation.
    LOOP AT initial_numbers REFERENCE INTO DATA(initial_number) GROUP BY ( key = initial_number->group count = GROUP SIZE ) ASCENDING REFERENCE INTO DATA(group_key).
      ...
      LOOP AT GROUP group_key REFERENCE INTO DATA(group_item).
        ...
        aggregated_item->min = nmin( val1 = aggregated_item->min val2 = group_item->number ).
        aggregated_item->max = nmax( val1 = aggregated_item->max val2 = group_item->number ).
      ENDLOOP.
