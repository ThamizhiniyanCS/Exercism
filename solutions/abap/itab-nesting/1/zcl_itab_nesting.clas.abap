...
CLASS zcl_itab_nesting IMPLEMENTATION.
  ...
  METHOD perform_nesting.
    LOOP AT artists REFERENCE INTO DATA(artist).
      APPEND INITIAL LINE TO nested_data REFERENCE INTO DATA(nested_level_1).

      nested_level_1->artist_id = artist->artist_id.
      nested_level_1->artist_name = artist->artist_name.

      LOOP AT albums REFERENCE INTO DATA(album) WHERE artist_id = artist->artist_id.
        APPEND INITIAL LINE TO nested_level_1->albums REFERENCE INTO DATA(nested_level_2).

        nested_level_2->album_id = album->album_id.
        nested_level_2->album_name = album->album_name.

        LOOP AT songs REFERENCE INTO DATA(song) WHERE artist_id = artist->artist_id AND album_id = album->album_id.
          APPEND INITIAL LINE TO nested_level_2->songs REFERENCE INTO DATA(nested_level_3).

          nested_level_3->song_id = song->song_id.
          nested_level_3->song_name = song->song_name.
